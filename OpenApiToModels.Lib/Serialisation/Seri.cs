...
public class Seri
{
    ...
    private void HandleWriteAllParams(OpenApiSchema schema)
    {
        ...
        {
            if (isFirstParam || _conf.IsNoNewlines) isFirstParam = false;
            else Tab().AppendLine();
    ...
    {
        if (_conf.IsCamelCase) name = name.ToTitleCase();
        ...
        switch ((param.Type, param.Format))
        {
            ...
            case ("boolean", _):
                ...
                break;
            case ("array", _):
                Tab().Append("public required ").Append(_conf.List).Append("TodoInnerType").Append('>').Append(name)
                    .AppendLine(" { get; set; }");
                break;
            case ("object", _):
    ...
    {
        Tab().Append(_conf.DefaultClassName).AppendLine(schema.Reference.Id);
        Tab().AppendLine("{");
        _depth++;
    }
    ...
    {
        if (_conf.IsCommentsActive == false || schema.Description is null) return;
    ...
    {
        if (_conf.IsCommentsActive == false || schema.Description is null) return;
        var exampleData = SeriHelpers.SerializeExampleData(
            schema.Example, _openApiDiagnostic?.SpecificationVersion ?? OpenApiSpecVersion.OpenApi3_0);
        EncloseInTagsCommented(exampleData);
    }

    private StringBuilder Tab() => _str.Append(SeriHelpers.TabRaw(_depth, _conf.Tab));
    ...
    private void EncloseInTagsCommented(string toEnclose, string startTag = "<example>", string endTag = "</example>")
    {
        ...
        if (toEnclose.Length + startTag.Length + endTag.Length + 2 < 110
            && toEnclose.Contains("\n") == false && _conf.IsEnforceSummaryNewlines == false)
        {
    ...
    {
        var serializer = new Seri(diagnostic);
        foreach (var schema in schemata) serializer.Add(schema);
        return serializer.Build();
    }
}

internal static class SeriHelpers
{
